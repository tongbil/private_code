...
methods: {
    ...
    pageCurrentChange: function(val) {
        //  console.log(`当前页: ${val}`);
       this.filter.offset = (val - 1) * this.filter.limit;
            ...
            if (valid) {
                ...
                instance.post(_this.url + "/update", _this.update
                    ...
                    if (response.data.result == "success") {
                        _this.$message.success('修改{{page_entiyname_cn}}成功！');
    ...
    createData: function() {
        var _this = this;
        // 主动校验
        _this.$refs.create.validate(function (valid) {
            if (valid) {
                _this.createLoading = true;
                var instance = axios_encrypt();
                instance.post(_this.url, _this.create)
                    .then(function (response) {
                        if (response.data.result == "success") {
    ...
    removeData: function(data) {
        var _this = this;
        _this.$confirm('此操作将永久删除{{page_entiyname_cn}} 【' + data.{{ page_entiyname }} + '】, 是否继续?', '提示', {
            type: 'warning'
        })
                .then(function () {
        // 向请求服务端删除
        var instance = axios_encrypt();
        instance.delete(_this.url + "{/id}", {
            params: {
                id: data.id
            }
        })
            .then(function (response) {
                _this.$message.success('成功删除了{{page_entiyname_cn}}【' + data.{{ page_entiyname }} + '】!');
        _this.getresultdatas();
    })
        .catch(function (response) {
...
removeresultdatas: function() {
    var _this = this;
    _this.$confirm('此操作将永久删除 ' + _this.selected.length + ' 个新闻栏位, 是否继续?', '提示', {
        type: 'warning'
    })
        .then(function () {
            ...
            var ids = [];
            //提取选中项的id
            _this.selected.forEach(function (user) {
                ids.push(user.id);
            });
            // 向请求服务端删除
            var instance = axios_encrypt();
            instance.delete(_this.url, {
                params: {
                    ids: ids.join(",")
                }
            }).then(function (response) {
                _this.$message.success('删除了' + _this.selected.length + '个{{page_entiyname_cn}}!');
                _this.getresultdatas();
            })
                .catch(function (response) {
                    _this.$message.error('删除失败!');
