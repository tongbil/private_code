var vm = new Vue({
    ...
    data: function () {
        ...
        return {
            ...
            {% endif %}
            {% if page_editbutton=="是" %}
            //更新{{page_entiyname_cn}}时候的form表单校验规则
            updateRules: {
            {% for formitem in updateForm %}
                {{formitem.name}}: [ 
                    {% if formitem.required %}
            ...
            //创建{{page_entiyname_cn}}动画显示
            createLoading: false,
            {% endif %}
            {% if page_editbutton=="是" %}
            dialogUpdateVisible: false, //编辑对话框的显示状态
            //更新{{page_entiyname_cn}}动画显示
            updateLoading: false,
            {% endif %}
    ...
    methods: {
        ...
        setCurrent:function(data) {
            ...
            resource.query({
                ...
                .then(function(response) {
                    this.$message.success('成功获取{{page_entiyname_cn}}' + data.username + '!');
                    this.update=Object.assign({},this.update,response.data);
                    this.dialogUpdateVisible = true;
                    this.dialogUpdateVisible = true;
                })
                .catch(function(response) {
                    this.$message.error('获取{{page_entiyname_cn}}失败!');
                });
            ...
            this.$refs.update.validate(function(valid) {
                if (valid) {
                    ...
                    resource.update({
                        ...
                        .then(function(response) {
                            this.$message.success('修改{{page_entiyname_cn}}成功！');
        ...
        // 获取{{page_entiyname_cn}}列表
        getresultdatas:function() {
            this.loading = true;
            var resource = this.$resource(this.url+"/page", this.searchform);
            resource.query(this.filter)
                .then(function(response) {
                    this.resultdatas = response.data.rows;
        ...
        removeData:function(data) {
            this.$confirm('此操作将永久删除{{page_entiyname_cn}} 【' + data.{{page_entiyname}} + '】, 是否继续?', '提示', {
                    type: 'warning'
                })
                .then(function() {
                    // 向请求服务端删除
                    ...
                    resource.delete({
                        ...
                        .then(function(response) {
                            this.$message.success('成功删除了{{page_entiyname_cn}}【' + data.{{page_entiyname}} + '】!');
                            this.getresultdatas();
                        })
                        .catch(function(response) {
        ...
        removeresultdatas:function() {
            this.$confirm('此操作将永久删除 ' + this.selected.length + ' 个{{page_entiyname_cn}}, 是否继续?', '提示', {
                    type: 'warning'
                })
                .then(function() {
                    ...
                    resource.remove({
                        ...
                        .then(function(response) {
                            this.$message.success('删除了' + this.selected.length + '个{{page_entiyname_cn}}!');
                            this.getresultdatas();
                        })
                        .catch(function(response) {
                            this.$message.error('删除失败!');
