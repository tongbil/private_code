...
//	});
	...
	$(document).on("mouseenter",".dropDown_hover",function(){
		$(this).addClass("open");
		var dataRel = $(this).children().eq(0).attr("data-rel");
		if(dataRel=="报告浏览"){
			$.ajax({
				type:"post",
				url :"ajaxReportHistory",
				success:function(msg){
					msg = JSON.parse(msg);
					var htmls = "";
					if(msg.items.length>8) {
						for(var i=0;i<8;i++){
							htmls +="<li><a href='javascript:void(0)' onclick='Hui_admin_tabNew(\"../../casereview/forwardXiangxi?id="+msg.items[i].adrId+"\",\"报告详情\");'> ";
							htmls +="<i class='Hui-iconfont c-primary'>&#xe623;";
							htmls +=	"</i>"+msg.items[i].reportCode +"&nbsp;&nbsp;&nbsp;"+msg.items[i].productName+"(<small style='color:#686868'>"+msg.items[i].times+"</small>)</a>";
							htmls +=	"</li>";
						}
					}else {
					...
					$("#reportHistory").html(htmls);
				}
				
			});
			//$("#reportHistory").html("<li><a href='javascript:void(0)' onclick='Hui_admin_tabNew('..//..//casereview//forwardXiangxi?id=111','报告详情');'> <i class='Hui-iconfont c-primary'>&#xe623;</i> 11111  222222 33333</a></li>");
		}
	});
